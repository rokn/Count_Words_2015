...
(0 == i) && fprintf(stderr, "%.*s\n", int(s.size()), s.c_str());
    ...
    { for(unsigned j = 0; j < WARMUPS; ++j)
    {
        ...
        { for(unsigned i = 0; i < ITERATIONS; ++i)
        {
            ...
            s.Format(TX("The first param '%s', and the second '%s', and the first again '%s'. Then the third '%s'. Finally, the last '%d'")
...
int main(int argc, char *argv[])
{
    ...
    defined(__SYNSOFT_DBS_DEBUG)
        puts("test.performance.three_strings_one_integer: " STLSOFT_COMPILER_LABEL_STRING);
#endif /* debug */
