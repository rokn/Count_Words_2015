...
static int main_(int argc, char** argv)
{
    ...
    { for(unsigned j = 0; j < WARMUPS; ++j)
    {
        ...
        { for(unsigned i = 0; i < ITERATIONS; ++i)
        {
            { for(unsigned k = 0; k != STLSOFT_NUM_ELEMENTS(args); ++k)
            {
                ...
                ss << args[k].name << ", " << args[k].age << ", " << args[k].height;
    ...
    { for(unsigned j = 0; j < WARMUPS; ++j)
    {
        ...
        { for(unsigned i = 0; i < ITERATIONS; ++i)
        {
            { for(unsigned k = 0; k != STLSOFT_NUM_ELEMENTS(args); ++k)
            {
                std::string s = boost::str( boost::format("%1%, %2%, %3%") % args[k].name % args[k].age % args[k].height);
    ...
    { for(unsigned j = 0; j < WARMUPS; ++j)
    {
        ...
        { for(unsigned i = 0; i < ITERATIONS; ++i)
        {
            { for(unsigned k = 0; k != STLSOFT_NUM_ELEMENTS(args); ++k)
            {
                ...
                Loki::SPrintf(s, ("%s, %d, %g"))(args[k].name)(args[k].age)(args[k].height);
    ...
    { for(unsigned j = 0; j < WARMUPS; ++j)
    {
        ...
        { for(unsigned i = 0; i < ITERATIONS; ++i)
        {
            { for(unsigned k = 0; k != STLSOFT_NUM_ELEMENTS(args); ++k)
            {
                ...
                s.Format(TEXT("%s, %d, %g"), args[k].name, args[k].age, args[k].height);
    ...
    { for(unsigned j = 0; j < WARMUPS; ++j)
    {
        ...
        { for(unsigned i = 0; i < ITERATIONS; ++i)
        {
            { for(unsigned k = 0; k != STLSOFT_NUM_ELEMENTS(args); ++k)
            {
                ...
                fastformat::fmt(s, TX("{0}, {1}, {2}"), args[k].name, args[k].age, args[k].height);
    ...
    { for(unsigned j = 0; j < WARMUPS; ++j)
    {
        ...
        { for(unsigned i = 0; i < ITERATIONS; ++i)
        {
            { for(unsigned k = 0; k != STLSOFT_NUM_ELEMENTS(args); ++k)
            {
                ...
                fastformat::write(s, args[k].name, ", ", args[k].age, ", ", args[k].height);
...
int main(int argc, char *argv[])
{
    ...
    defined(__SYNSOFT_DBS_DEBUG)
        puts("test.performance.name_age_height: " STLSOFT_COMPILER_LABEL_STRING);
#endif /* debug */
